...
struct ScreenRecorder {
    ...
    init(url: URL, displayID: CGDirectDisplayID, cropRect: CGRect?, mode: RecordMode, selectedWindow: SCWindow?) async throws {
        ...
        var audioSettings = [AVSampleRateKey : 48000, AVNumberOfChannelsKey : 2]
        audioSettings[AVFormatIDKey] = Int(kAudioFormatMPEG4AAC)
        audioSettings[AVEncoderBitRateKey] = audioQuality.rawValue * 1000
        ...
        configuration.sampleRate = audioSettings["AVSampleRateKey"]!
        configuration.channelCount = audioSettings["AVNumberOfChannelsKey"]!
...
//            configuration.colorSpaceName = CGColorSpace.displayP3
        ...
        /*
          if selectedWindow not nil record only specific window like chrome
         */

        if let choosed = selectedWindow {
            let selectedWindowFilter = SCContentFilter(desktopIndependentWindow: choosed)
            stream = SCStream(filter: selectedWindowFilter, configuration: configuration, delegate: nil)
        } else {
    ...
    private class StreamOutput: NSObject, SCStreamOutput {
        ...
        func stream(_ stream: SCStream, didOutputSampleBuffer sampleBuffer: CMSampleBuffer, of type: SCStreamOutputType) {
            ...
            print("SCStreamOutputType type: \(type.rawValue)")
